...
class SkipPageSelector(QtWidgets.QDialog):
    ...
    def __init__(self, wizard_obj, output_dict, *args, **kwargs):
        ...
        data = [w.page_links[suffix].title() for suffix in self.avail_page_suffixes]
...
class ReportWizard(QtWidgets.QWizard):
    def __init__(self):
        ...
        for k in self.page_name_list:
            self.page_links[k] = self.findChild(
                QtWidgets.QWizardPage, f"wizardPage_{k}"
            )
    ...
    def loadSettings(self):
        ...
        saved_folderpath = self.settings.value("config_folderpath", os.getcwd())
        if not os.path.exists(saved_folderpath):
            saved_folderpath = os.getcwd()
        self._settings["config_folderpath"] = saved_folderpath

        saved_seed_filepath = self.settings.value("seed_config_filepath", "")
        if saved_seed_filepath and not os.path.exists(saved_seed_filepath):
            saved_seed_filepath = ""
        self._settings["seed_config_filepath"] = saved_seed_filepath
    ...
    def saveSettings(self):
        ...
        self.settings.setValue("config_folderpath", self._settings["config_folderpath"])

        self.settings.setValue(
            "seed_config_filepath", self._settings["seed_config_filepath"]
        )
    ...
    def update_common_remote_opts(self):
        ...
        if "common_remote_opts" in self.conf["nonlin"]:
            ...
            self.common_remote_opts.update(self.conf["nonlin"]["common_remote_opts"])
