...
def get_n_free_cores(partition="normal"):
    ...
    node_names = np.unique(
        [e for k, v in sinfo[partition].items() for e in v if e not in non_ok_nodes]
    )
...
def get_avail_adjusted_ncores(ncores_max, ncores_min, partition=None):
    ...
    for _ in range(3):
        ...
        print(
            f"* Failed to obtain # of available cores. Will assume max. requested cores ({ncores_max}) available."
        )
        ...
        if k in (
            ...
            if v is None:
                ...
                if v not in ("gpu", "gpu:2"):
                    raise ValueError(
                        '--gres must be "gpu" for 1 GPU or "gpu:2" for 2 GPUs'
                    )
                slurm_opts[k] = "--{}={}".format(k, v)
...
def file_exists_w_nfs_cache_flushing(filepath, nMaxTry=5, interval=3.0):
    ...
    for _ in range(nMaxTry):
        ...
        print(
            f'Waited file "{filepath}" to show up for {time.time()-tFileWait:.3f} [s]'
        )
...
def stop_mpi_executor(executor_d, del_log_files=True):
    ...
    if del_log_files:
        ...
        for fp in [executor_d["slurm_out_filepath"], executor_d["slurm_err_filepath"]]:
            try:
                fp.unlink()
            except:
                pass
...
def submit_job_to_mpi_executor(
    executor_d,
    module_name,
    func_name,
    param_list,
    ...
    if print_stdout:
        h_dt_setup = get_human_friendly_time_duration_str(dt["setup"], fmt=".3f")
        h_dt_run = get_human_friendly_time_duration_str(dt["run"], fmt=".3f")
    ...
    for fp in [
        input_filepath,
        input_ready_filepath,
        output_ready_filepath,
        input_d["output_filepath"],
    ]:
        try:
            fp.unlink()
...
def _wait_for_mpi_exec_job_completion(
    ...
    while not finished:
        for fp in tmp_dirpath.glob(output_ready_filename):
            ...
            if state != "R":
                raise RuntimeError(
                    f"MPI executor job NOT in the running state: {state}"
                )
