...
def print_load():
    ...
    nMaxNodeNameLen = max(
        [len(s) for s in list(list(zip(*parsed))[0]) + [v[0] for v in GROUPED_NODES]]
    )
...
def _notify_on_num_free_cores_change(
    ...
    print(
        f'When # of free cores change on the partition "{partition}", '
    ...
    while True:
        ...
        if (n_free_now != n_free_prev) and (n_free_now >= n_min_free):
            ...
            if send_email:
                ...
                sendRunCompleteMail(
                    f'"{partition}": # of free cores = {n_free_now}', email_content
                )
        ...
        if sleep_hr_ini == sleep_hr_fin:
            ...
            if d.hour >= sleep_hr_ini:
                # Go dormant
                wakeup_d = datetime.now().astimezone()
                wakeup_d = wakeup_d.replace(
                    day=d.day + 1, hour=sleep_hr_fin, minute=0, second=0
                )
                actual_interval = (wakeup_d - d).total_seconds()
                dormant = True
