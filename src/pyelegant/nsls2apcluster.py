...
def set_slurm_abs_time_limit(abs_time_limit_str, partition=None):
    ...
    try:
        ...
        raise ValueError(
            (
                'Invalid format for "abs_time_limit_str". '
            ...
            if "sbatch_err_check_tree" not in remote_opts:
                ...
                if remote_opts["sbatch_err_check_tree"] is None:
                    ...
                    _update_sbatch_err_check_tree_kwargs(
                        remote_opts["sbatch_err_check_tree"],
                        output_filepaths,
                        slurm_err_filepath,
                        abort_info,
                    )
...
def _sbatch(
    sbatch_sh_filepath, job_name, exit_right_after_submission=False, err_log_check=None
):
    """"""
...
def _check_slurm_err_log(err_contents, abort_info):
    ...
    if "double free or corruption" in err_contents:
        ...
        p = Popen(
            'squeue -o "%.7i %.9P %.8j %.8u %.2t %.10M %.10L %.6D %.4C %R"',
            stdout=PIPE,
            stderr=PIPE,
            shell=True,
        )
        ...
        "in mode w: File exists" in err_contents
    ):  # and \
        ...
        (not err_contents.startswith("Error:"))
        and ("CANCELLED AT" in err_contents)
        and ("DUE TO TIME LIMIT" in err_contents)
    ):
...
""".format(
        slurm_switches=" ".join([v for v in slurm_opts.values()])
    )
...
def run_mpi_python(
    ...
    input_filepath, output_filepath, mpi_sub_sh_filepath = gen_mpi_submit_script(
        ...
        ntasks=remote_opts.get("ntasks", 50),
        x11=remote_opts.get("x11", False),
    ...
    while True:
        p = Popen(
            "sbatch {0}".format(mpi_sub_sh_filepath),
            stdout=PIPE,
            stderr=PIPE,
            shell=True,
        )
    ...
    info = wait_for_completion(
        job_ID_str,
        remote_opts.get("status_check_interval", 3.0),
        err_log_check=err_log_check,
    )
    ...
    if remote_opts.get("del_job_out_file", True):
        try:
            os.remove(f"{job_name}.{job_ID_str}.out")
        except:
            pass
    if remote_opts.get("del_job_err_file", True):
        try:
            os.remove(f"{job_name}.{job_ID_str}.err")
        except:
            pass
...
def srun_python_func(
    ...
    nodelist = (
        ""
        if "nodelist" not in remote_opts
        else f'--nodelist={",".join(remote_opts["nodelist"])}'
    )
    exclude = (
        ""
        if "exclude" not in remote_opts
    ...
    cmd = (
        f"srun -c {cpus_per_task:d} -J {job_name} -p {partition} {x11} {timelimit_str} "
...
srun {mpi_compiler_opt} python -m mpi4py.futures {main_script_path} _mpi_starmap {input_filepath}
    ...
    _exclude = "" if exclude == [] else "#SBATCH --exclude={}".format(",".join(exclude))
    ...
    contents = contents_template.format(
        ...
        main_script_path=__file__[:-3] + "_mpi_script.py",
...
def starmap_async(
    ...
    if "job_name" not in slurm_opts:
        ...
        slurm_opts["job_name"] = "--job-name={}".format(DEFAULT_REMOTE_OPTS["job_name"])
